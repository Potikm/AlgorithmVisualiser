...
const Algos = () => {
    ...
    return (
        <div>
            ...
            {alghorithm === "" ? <h1>Choose the Algo ;)</h1> : <Bars alghorithm={alghorithm} key={alghorithm} play={play} setPlay={setPlay} pause={pause} timer={timer} setSorted={setSorted} spawn={spawn} setSpawn={setSpawn} reload={reload} setReload={setReload} setPause={setPause} restart={restart} setRestart={setRestart} />}
            ...
            <div className="flex">
                ...
                {alghorithm === "" ? null : <div><button className='restartBtn' style={play || sorted ? {} : { pointerEvents: 'none', opacity: '0.3' }} onClick={() => setRestart(true)}>{restartText}</button> <button className='reloadBtn' onClick={() => setReload(true)}>{btnText}</button> </div>}
            </div>
            <Helper type={alghorithm} />

            <Settings pause={pause} setPause={setPause} setTimer={setTimer} algo={alghorithm} sorted={sorted} />
